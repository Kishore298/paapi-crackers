      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className={`card overflow-hidden transition-all ${activeStep !== 1 ? 'opacity-70' : ''}`}>
            <div 
              className={`p-4 flex items-center justify-between cursor-pointer ${activeStep === 1 ? 'bg-primary-lighter/50 border-b border-border' : ''}`}
              onClick={() => setActiveStep(1)}
            >
              <div className="flex items-center gap-3">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold ${activeStep === 1 ? 'bg-primary text-white' : 'bg-gray-200 text-text-secondary'}`}>1</div>
                <h2 className="text-lg font-bold text-text-primary flex items-center gap-2"><User size={20} className="text-primary"/> Personal Info</h2>
              </div>
            ...
            {activeStep === 1 && (
              <div className="p-6 space-y-4">
                ...
                <button onClick={() => handleNextStep(1)} disabled={loading} className="btn-primary mt-4 w-full md:w-auto">Continue to Address</button>
              </div>
            )}
          </div>
          ...
          <div className={`card overflow-hidden transition-all ${activeStep !== 2 ? 'opacity-70' : ''}`}>
             <div 
              className={`p-4 flex items-center justify-between cursor-pointer ${activeStep === 2 ? 'bg-primary-lighter/50 border-b border-border' : ''}`}
              onClick={() => activeStep > 1 && setActiveStep(2)}
            >
              <div className="flex items-center gap-3">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold ${activeStep === 2 ? 'bg-primary text-white' : 'bg-gray-200 text-text-secondary'}`}>2</div>
                <h2 className="text-lg font-bold text-text-primary flex items-center gap-2"><MapPin size={20} className="text-primary"/> Shipping & GST</h2>
              </div>
            ...
            {activeStep === 2 && (
              <div className="p-6 space-y-4">
                <div>
                  ...
                  <textarea name="address" value={formData.address} onChange={handleChange} rows={3} className="input-field resize-none" placeholder="House/Flat No., Street Name, Area" />
                </div>
                ...
                <div className="flex gap-3 mt-6">
                  <button onClick={() => setActiveStep(1)} className="btn-outline flex-1 md:flex-none">Back</button>
                  <button onClick={() => handleNextStep(2)} className="btn-primary flex-1 md:flex-none">Continue to Payment</button>
                </div>
              </div>
            )}
          </div>
          ...
          <div className={`card overflow-hidden transition-all ${activeStep !== 3 ? 'opacity-70' : ''}`}>
             <div 
              className={`p-4 flex items-center justify-between cursor-pointer ${activeStep === 3 ? 'bg-primary-lighter/50 border-b border-border' : ''}`}
            >
              <div className="flex items-center gap-3">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold ${activeStep === 3 ? 'bg-primary text-white' : 'bg-gray-200 text-text-secondary'}`}>3</div>
                <h2 className="text-lg font-bold text-text-primary flex items-center gap-2"><CreditCard size={20} className="text-primary"/> Payment Method</h2>
              </div>
            </div>
